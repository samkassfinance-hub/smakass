...
// ── CONFIG ──────────────────────────────────────────────────
let API_BASE = '/api'; // Same deployment, relative path
...
setTimeout(() => {
  ...
  if (loader && loader.style.display !== 'none') {
    console.error('❌ EMERGENCY: App failed to initialize after 3s, forcing loader hide');
    loader.style.display = 'none';
    // Force show auth screen as fallback
    const authScreen = document.getElementById('auth-screen');
    if (authScreen) {
      authScreen.style.display = 'block';
      console.log('✅ EMERGENCY: Showing auth screen as fallback');
    }
  }
}, 3000);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(() => {
    ...
    try {
      ...
      console.error('❌ BOOT: Fatal error in init():', error);
      // Force hide loader and show auth screen on error
      const loader = document.getElementById('app-loading');
      if (loader) loader.style.display = 'none';
      const authScreen = document.getElementById('auth-screen');
...
function closeChatbot() {
  ...
  if (chatbotInterface && chatbotIconContainer) {
...
function sendMessage() {
  ...
  // Call AI with client-specific enhancement
  callKassGPTAPI(message).then(response => {
    hideTypingIndicator();
    ...
    hideTypingIndicator();
    // Fallback
    const fallbackResponse = processUserMessageWithAI(message);
...
// Standalone intelligent chatbot - NO external APIs
async function callKassGPTAPI(message) {
  // Check for client-specific queries first
  const clients = Store.clients();
  const loans = Store.loans();
  const payments = Store.payments();
  
  // Try client-specific handler if available
  if (typeof enhanceClientSpecificQueries === 'function') {
    const clientResponse = enhanceClientSpecificQueries(message, clients, loans, payments);
    if (clientResponse) {
      return clientResponse; // Return client-specific answer
    }
  }
  
  // Fall back to general processing
  return processUserMessageWithAI(message);
}

// Enhanced AI processing with comprehensive app knowledge
function processUserMessageWithAI(message) {
  ...
  const weeklyLoans = loans.filter(l => l.type === 'weekly');
  const monthlyLoans = loans.filter(l => l.type === 'monthly');
  ...
  const overduePayments = payments.filter(p => p.dueDate && p.dueDate < today && p.status === 'pending');
  ...
  // CLIENT QUERIES
  if (lowerMessage.includes('client') || lowerMessage.includes('customer') || lowerMessage.includes('வாடிக்கையாளர்')) {
    if (lowerMessage.includes('how many') || lowerMessage.includes('total') || lowerMessage.includes('count') || lowerMessage.includes('எத்தனை')) {
      const clientsWithLoans = clients.filter(c => loans.some(l => l.clientId === c.id));
      return `📊 **Client Summary:**\n\nTotal Clients: **${clients.length}**\nClients with Loans: **${clientsWithLoans.length}**\nClients without Loans: **${clients.length - clientsWithLoans.length}**\n\n💡 Manage all clients in the **Clients** tab.`;
    }
    
    if (lowerMessage.includes('list') || lowerMessage.includes('show') || lowerMessage.includes('who')) {
      if (clients.length === 0) {
        return `You don't have any clients yet!\n\n➕ **Add your first client:**\n1. Go to Clients tab\n2. Click "+ Add Client"\n3. Fill in details\n4. Save\n\nStart building your client base! 💼`;
      }
      const clientList = clients.slice(0, 10).map((c, i) => `${i + 1}. ${c.name} - ${c.phone || 'No phone'}`).join('\n');
      return `👥 **Your Clients** (showing ${Math.min(10, clients.length)} of ${clients.length}):\n\n${clientList}\n\n💡 View full details in the Clients tab.`;
    }
    
    if (lowerMessage.includes('add') || lowerMessage.includes('create') || lowerMessage.includes('new')) {
      return `➕ **How to Add a Client:**\n\n**Step 1:** Tap **Clients** tab (bottom)\n**Step 2:** Click **"+ Add Client"** button\n**Step 3:** Enter details:\n  • Name (required)\n  • Phone number\n  • Address\n  • Occupation\n**Step 4:** Click **Save**\n\n✅ Client added! Ready for loans.`;
    }
  }
  
  // LOAN QUERIES
  if (lowerMessage.includes('loan') || lowerMessage.includes('கடன்')) {
    if (lowerMessage.includes('how many') || lowerMessage.includes('total') || lowerMessage.includes('count') || lowerMessage.includes('எத்தனை')) {
      return `💰 **Loan Statistics:**\n\n**Total Loans:** ${loans.length}\n**Active:** ${activeLoans.length}\n**Completed:** ${completedLoans.length}\n\n**By Type:**\n• Weekly: ${weeklyLoans.length}\n• Monthly: ${monthlyLoans.length}\n\n**Financial:**\n• Total Disbursed: ₹${totalLoanAmount.toLocaleString()}\n• Collected: ₹${totalCollected.toLocaleString()}\n• Outstanding: ₹${(totalLoanAmount - totalCollected).toLocaleString()}\n\n📊 Track everything in **Loans** tab!`;
    }
    
    if (lowerMessage.includes('list') || lowerMessage.includes('show') || lowerMessage.includes('active')) {
      if (activeLoans.length === 0) {
        return `No active loans currently.\n\n💡 ${loans.length > 0 ? `You have ${completedLoans.length} completed loans.` : 'Create your first loan in the Loans tab!'}`;
      }
      const loanList = activeLoans.slice(0, 5).map((l, i) => {
        const client = clients.find(c => c.id === l.clientId);
        return `${i + 1}. ${client?.name || 'Unknown'} - ₹${parseFloat(l.principal).toLocaleString()} (${l.type})`;
      }).join('\n');
      return `💰 **Active Loans** (showing ${Math.min(5, activeLoans.length)} of ${activeLoans.length}):\n\n${loanList}\n\n💡 View details in **Loans** tab.`;
    }
    
    if (lowerMessage.includes('add') || lowerMessage.includes('create') || lowerMessage.includes('new') || lowerMessage.includes('how')) {
      return `💰 **How to Create a Loan:**\n\n**Step 1:** Go to **Loans** tab\n**Step 2:** Click **"+ Add Loan"**\n**Step 3:** Select **client** from dropdown\n**Step 4:** Enter loan details:\n  • Principal amount\n  • Interest rate (%)\n  • Interest type (percentage/flat)\n  • Duration (number)\n  • Loan type (weekly/monthly)\n  • Start date\n**Step 5:** Click **"Create Loan"**\n\n✅ EMI schedule auto-generated!\n📅 Payment reminders set up!`;
    }
    
    if (lowerMessage.includes('calculate') || lowerMessage.includes('emi') || lowerMessage.includes('interest')) {
      return `🧮 **Loan Calculator:**\n\nThe app automatically calculates:\n• **EMI Amount** based on principal + interest\n• **Payment Schedule** (weekly/monthly)\n• **Total Interest**\n• **Total Amount** to repay\n\n💡 All calculations happen when you create a loan!\n\n**Formula used:**\nSimple Interest: P × R × T / 100\nEMI: (Principal + Interest) / Number of Installments`;
    }
  }
  
  // PAYMENT QUERIES
  if (lowerMessage.includes('payment') || lowerMessage.includes('collection') || lowerMessage.includes('பணம்')) {
    if (lowerMessage.includes('how many') || lowerMessage.includes('total') || lowerMessage.includes('count')) {
      return `💳 **Payment Summary:**\n\n**Total Payments:** ${payments.length}\n**Paid:** ${paidPayments.length} (₹${totalCollected.toLocaleString()})\n**Pending:** ${pendingPayments.length} (₹${totalPending.toLocaleString()})\n**Overdue:** ${overduePayments.length} (₹${overdueAmount.toLocaleString()})\n\n**Today's Collection:** ₹${todayCollection.toLocaleString()}\n\n📊 Manage in **Payments** or **Collections** tab.`;
    }
    
    if (lowerMessage.includes('pending') || lowerMessage.includes('due') || lowerMessage.includes('upcoming')) {
      if (pendingPayments.length === 0) {
        return `✅ **All caught up!**\n\nNo pending payments right now.\n\n${paidPayments.length > 0 ? `You've collected ${paidPayments.length} payments totaling ₹${totalCollected.toLocaleString()}.` : ''}`;
      }
      const upcomingList = pendingPayments.slice(0, 5).map((p, i) => {
        const loan = loans.find(l => l.id === p.loanId);
        const client = clients.find(c => c.id === loan?.clientId);
        return `${i + 1}. ${client?.name || 'Unknown'} - ₹${parseFloat(p.amount).toLocaleString()} (Due: ${p.dueDate || 'Not set'})`;
      }).join('\n');
      return `📋 **Pending Payments** (${Math.min(5, pendingPayments.length)} of ${pendingPayments.length}):\n\n${upcomingList}\n\n💡 Track in **Collections** tab.`;
    }
    
    if (lowerMessage.includes('today') || lowerMessage.includes('இன்று')) {
      if (todayCollection === 0) {
        return `📅 **Today's Collections:**\n\nNo payments received today yet.\n\n💡 Check **Collections** tab for pending payments.`;
      }
      return `📅 **Today's Collections:**\n\n**Payments Received:** ${todayPayments.length}\n**Amount:** ₹${todayCollection.toLocaleString()}\n\n${todayPayments.length > 0 ? '✅ Great work today!' : ''}`;
    }
    
    if (lowerMessage.includes('record') || lowerMessage.includes('mark') || lowerMessage.includes('add')) {
      return `💳 **Record a Payment:**\n\n**Method 1: From Collections**\n1. Go to **Collections** tab\n2. Find the payment\n3. Click **"Mark as Paid"**\n4. Confirm\n\n**Method 2: From Loan**\n1. Go to **Loans** tab\n2. Click on the loan\n3. Find payment in EMI schedule\n4. Click **"Mark Paid"**\n\n✅ Payment recorded!\n📄 Generate receipt automatically.`;
    }
  }
  ...
  if (lowerMessage.includes('overdue') || lowerMessage.includes('late') || lowerMessage.includes('missed')) {
    if (overduePayments.length === 0) {
      return `✅ **No Overdue Payments!**\n\nExcellent collection management!\n\n📊 **Current Status:**\n• All payments on track\n• ${pendingPayments.length} pending (not overdue)\n• Collection rate: ${payments.length > 0 ? Math.round((paidPayments.length / payments.length) * 100) : 0}%\n\nKeep up the great work! 🎉`;
    }
    
    const overdueList = overduePayments.slice(0, 5).map((p, i) => {
      const loan = loans.find(l => l.id === p.loanId);
      const client = clients.find(c => c.id === loan?.clientId);
      const daysLate = Math.floor((new Date() - new Date(p.dueDate)) / (1000 * 60 * 60 * 24));
      return `${i + 1}. ${client?.name || 'Unknown'} - ₹${parseFloat(p.amount).toLocaleString()} (${daysLate} days late)`;
    }).join('\n');
    
    return `🔔 **Overdue Payments Alert!**\n\n**Total Overdue:** ${overduePayments.length} payments\n**Amount:** ₹${overdueAmount.toLocaleString()}\n\n**Top Priority** (${Math.min(5, overduePayments.length)} of ${overduePayments.length}):\n${overdueList}\n\n💡 **Actions:**\n• Contact clients via phone\n• Send payment reminders\n• Check Collections tab for details`;
  }
  
  // STATISTICS & REPORTS
  if (lowerMessage.includes('statistic') || lowerMessage.includes('report') || lowerMessage.includes('analytics') || lowerMessage.includes('summary') || lowerMessage.includes('overview')) {
    const collectionRate = payments.length > 0 ? Math.round((paidPayments.length / payments.length) * 100) : 0;
    const avgLoanSize = loans.length > 0 ? Math.round(totalLoanAmount / loans.length) : 0;
    
    return `📊 **SamKass Analytics Dashboard**\n\n**👥 Clients:** ${clients.length}\n\n**💰 Loans:**\n• Total: ${loans.length} (Active: ${activeLoans.length})\n• Total Disbursed: ₹${totalLoanAmount.toLocaleString()}\n• Average Size: ₹${avgLoanSize.toLocaleString()}\n\n**💳 Collections:**\n• Total Collected: ₹${totalCollected.toLocaleString()}\n• Pending: ₹${totalPending.toLocaleString()}\n• Overdue: ₹${overdueAmount.toLocaleString()}\n• Collection Rate: ${collectionRate}%\n\n**📅 Today:**\n• Collected: ₹${todayCollection.toLocaleString()}\n\n💡 Export detailed reports from **Settings**!`;
  }
  
  // RECEIPT GENERATION
  if (lowerMessage.includes('receipt') || lowerMessage.includes('invoice') || lowerMessage.includes('pdf')) {
    return `📄 **Generate Receipts:**\n\n**For Single Payment:**\n1. Go to **Payments** tab\n2. Click on the payment\n3. Click **"Generate Receipt"**\n4. Choose:\n   • Download PDF\n   • Share via WhatsApp\n   • Email\n\n**For Loan Statement:**\n1. Go to **Loans** tab\n2. Select loan\n3. Click **"Download Statement"**\n\n✨ **Features:**\n• Professional design\n• Company branding\n• Payment details\n• Loan summary\n• Digital signature\n\nAll receipts saved automatically!`;
  }
  
  // EXPORT & BACKUP
  if (lowerMessage.includes('export') || lowerMessage.includes('download') || lowerMessage.includes('backup') || lowerMessage.includes('save')) {
    return `💾 **Export & Backup:**\n\n**From Settings Tab:**\n\n**1. PDF Export**\n• Complete data report\n• All clients, loans & payments\n• Professional format\n\n**2. Excel/CSV**\n• Spreadsheet format\n• Easy to analyze\n• Import to other tools\n\n**3. Cloud Backup (Pro)**\n• Sync to Supabase\n• Auto-backup daily\n• Access from any device\n• Restore anytime\n\n**4. JSON Export**\n• Full database export\n• For developers\n• Easy migration\n\n🔒 Your data is encrypted & secure!`;
  }
  
  // APP FEATURES
  if (lowerMessage.includes('feature') || lowerMessage.includes('what can') || lowerMessage.includes('how to use')) {
    return `✨ **SamKass Features:**\n\n**📱 Core:**\n• Client Management\n• Loan Creation & Tracking\n• EMI Auto-calculation\n• Payment Recording\n• Collections Tracking\n• Receipt Generation\n\n**📊 Analytics:**\n• Dashboard Stats\n• Financial Reports\n• Payment History\n• Overdue Alerts\n• Collection Rate\n\n**🎨 Customization:**\n• Dark/Light Theme\n• English/Tamil Language\n• Currency Format\n• Date Format\n\n**💾 Data:**\n• Local Storage\n• Cloud Sync (Pro)\n• PDF Export\n• Excel Export\n• Auto Backup\n\n**🔒 Security:**\n• PIN Lock\n• Data Encryption\n• Secure Auth\n• Recycle Bin\n\nExplore all features! 🚀`;
  }
  
  // SUBSCRIPTION
  if (lowerMessage.includes('subscription') || lowerMessage.includes('plan') || lowerMessage.includes('upgrade') || lowerMessage.includes('pro') || lowerMessage.includes('price')) {
    const currentPlan = settings.plan || 'free';
    return `💎 **SamKass Plans:**\n\n**🆓 FREE PLAN** ${currentPlan === 'free' ? '← You are here' : ''}\n• Up to 5 clients\n• Unlimited loans & payments\n• Local storage\n• Basic features\n• PDF receipts\n\n**⭐ PRO PLAN** ${currentPlan === 'pro' ? '← You are here' : ''}\n• **Unlimited** clients\n• Cloud backup & sync\n• Advanced analytics\n• Custom branding\n• WhatsApp integration\n• Priority support\n• Data export (Excel)\n• Multi-device access\n\n**💰 Pricing:**\n• Monthly: ₹299/month\n• Yearly: ₹2,999/year (Save 17%!)\n\n${currentPlan === 'free' ? '💡 Upgrade in **Subscription** tab!' : '✅ Enjoying Pro benefits!'}\n\n🎁 7-day free trial available!`;
  }
  
  // HELP & SUPPORT
  if (lowerMessage.includes('help') || lowerMessage.includes('support') || lowerMessage.includes('contact') || lowerMessage.includes('problem') || lowerMessage.includes('issue')) {
    return `💬 **Help & Support:**\n\n**📧 Contact Us:**\n• Email: eniyavanan04@gmail.com\n• WhatsApp: +91 7904987242\n• Response: Within 24 hours\n\n**📚 Resources:**\n• Video Tutorials (in app)\n• User Guide\n• FAQ Section\n• Feature Walkthrough\n\n**🐛 Report Bug:**\nInclude:\n• Screenshot\n• What happened\n• Steps to reproduce\n• Your device/browser\n\n**💡 Feature Request:**\nSuggest new features!\nWe're always improving.\n\n**⚡ Quick Help:**\nAsk me specific questions like:\n• "How to add a loan?"\n• "Calculate EMI"\n• "Export data"\n\nI'm here 24/7! 😊`;
  }
  
  // SETTINGS
  if (lowerMessage.includes('setting') || lowerMessage.includes('theme') || lowerMessage.includes('language') || lowerMessage.includes('customize')) {
    return `⚙️ **App Settings:**\n\n**From Settings Tab:**\n\n**🎨 Appearance:**\n• Theme: Light / Dark\n• Current: ${settings.theme || 'light'}\n\n**🌍 Language:**\n• English\n• Tamil (தமிழ்)\n• Current: ${settings.lang === 'ta' ? 'Tamil' : 'English'}\n\n**💰 Format:**\n• Currency: ₹ (Rupees)\n• Date format: DD/MM/YYYY\n• Number format: Indian\n\n**🔔 Notifications:**\n• Payment reminders\n• Overdue alerts\n• Daily summary\n\n**🔒 Security:**\n• Change PIN\n• Enable 2FA\n• Auto-lock timer\n\n**💾 Data:**\n• Auto-backup\n• Sync frequency\n• Storage location\n\nCustomize your experience!`;
  }
  
  // DEFAULT INTELLIGENT RESPONSE
  return `👋 Hello! I'm your SamKass assistant!\n\n📊 **Your Quick Stats:**\n• **${clients.length}** clients\n• **${activeLoans.length}** active loans\n• **₹${totalPending.toLocaleString()}** pending collections\n${overduePayments.length > 0 ? `• **${overduePayments.length}** overdue payments ⚠️` : ''}\n\n💡 **Ask me about:**\n• "Show me overdue payments"\n• "How many clients do I have?"\n• "Today's collections"\n• "How to create a loan?"\n• "Export my data"\n• "Loan statistics"\n• "Generate receipt"\n• "Upgrade to Pro"\n\n**Or ask anything else!** I know everything about your SamKass app. 🚀`;
}
